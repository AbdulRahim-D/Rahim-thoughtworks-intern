...
var allBooksList = books;
var displayedBooks = [...allBooksList];
...
function showBooks() {
  ...
  var start = (currentPageNumber - 1) * itemsPerPage;
  var end = start + itemsPerPage;
  var currentTenBooks = displayedBooks.slice(start, end);

  for (var i = 0; i < currentTenBooks.length; i++) {
    var book = currentTenBooks[i];
    var myRow = document.createElement("tr");

    var coverPage = document.createElement("td");
    if (book.thumbnailUrl) {
      var img = document.createElement("img");
      img.src = book.thumbnailUrl;
      coverPage.appendChild(img);
    } else {
      coverPage.innerText = "Unavailable";
    }
    myRow.appendChild(coverPage);

    var title = document.createElement("td");
    title.innerText = book.title || "Unavailable";
    myRow.appendChild(title);

    var bookisbn = document.createElement("td");
    bookisbn.innerText = book.isbn || "Unavailable";
    myRow.appendChild(bookisbn);

    var totalpages = document.createElement("td");
    totalpages.innerText = book.pageCount || "0";
    myRow.appendChild(totalpages);

    var bookdate = document.createElement("td");
    if (book.publishedDate) {
      bookdate.innerText = book.publishedDate.$date.slice(0, 10);
    } else {
      bookdate.innerText = "Unavailable";
    }
    myRow.appendChild(bookdate);

    var shrtdes = document.createElement("td");
    var sdes = book.shortDescription || "Unavailable";
    shrtdes.innerText = sdes.slice(0, 100);
    shrtdes.classList.add("paragraph");
    myRow.appendChild(shrtdes);

    var lngdes = document.createElement("td");
    var ldes = book.longDescription || "Unavailable";
    lngdes.innerText = ldes.slice(0, 100);
    lngdes.classList.add("paragraph");

    myRow.appendChild(lngdes);

    var bookstate = document.createElement("td");
    bookstate.innerText = book.status || "Unavailable";
    myRow.appendChild(bookstate);
    var bookauth = document.createElement("td");
    bookauth.innerText = book.authors ? book.authors.join(", ") : "Unavailable";
    myRow.appendChild(bookauth);
    var category = document.createElement("td");
    category.innerText = book.categories
      ? book.categories.join(", ")
      : "Unavailable";
    myRow.appendChild(category);
    var value = document.createElement("td");
    value.innerText = book.price || "Unavailable";
    myRow.appendChild(value);
    var typeofcurrency = document.createElement("td");
    typeofcurrency.innerText = book.currency || "Unavailable";
    myRow.appendChild(typeofcurrency);
    var bookdiscount = document.createElement("td");
    bookdiscount.innerText = book.discount || "0";
    myRow.appendChild(bookdiscount);
    var disunit = document.createElement("td");
    disunit.innerText = book.discountUnits || "Unavailable";
    myRow.appendChild(disunit);
...
function searchData() {
  ...
  displayedBooks = allBooksList.filter(function (b) {
...
function sortData(column) {
  ...
  displayedBooks.sort(function (a, b) {
    ...
    if (column === "publishedDate") {
      valA = a[column] ? a[column].$date : "";
      valB = b[column] ? b[column].$date : "";
    }
  ...
  for (var i = 0; i < ids.length; i++) {
    var el = document.getElementById("arrow-" + ids[i]);
    if (el) el.innerText = "";
  }
  document.getElementById("arrow-" + column).innerText = isAscending
    ? " ⬆️"
    : " ⬇️";
  showBooks();
}
...
function goNext() {
  if (currentPageNumber * itemsPerPage < displayedBooks.length) {
    currentPageNumber++;
    showBooks();
  }
}
